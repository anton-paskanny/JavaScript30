document.addEventListener('DOMContentLoaded', e => {
    ...
    console.log('Array we are going to work with: ');
    ...
    const allYears = inventors.reduce((acc, currVal) => acc + (currVal.passed - currVal.year), 0);
    ...
    const alpha = people.sort((a, b) => {
        // First variant - split string to array with first and last names
        const [aLast, aFirst] = a.split(', ');
        const [bLast, bFirst] = b.split(', ');

        return aLast > bLast ? 1 : -1;

        // Second variant - cut substring from full string and use it in comparison.
